...
player_piece=pygame.image.load(".//res//othello10_res//white.png") #BLACK piece
enemy_piece=pygame.image.load(".//res//othello10_res//black_piece.png") #WHITE piece 
white_moved = pygame.image.load(".//res//othello10_res//white1_moved.png")
legal_move_indicator = pygame.image.load(".//res//othello10_res//move_indicator.png") #red dot
undo = pygame.image.load(".//res//othello10_res//undo.png") #undo 
pass_button = pygame.image.load(".//res//othello10_res//pass.png")
...
def get_moves(layout,r,c): 
    ...
    for i in range(r): 
        for j in range(c): 
            if layout[i][j]=="?":
                moves.append(position_letter_grid_10[i][j])
    return moves 
...
def check_game():
    if Player_Piece_Count == 0: #computer wins 
        ...
        if Player_Piece_Count+Enemy_Piece_Count == 100: #if the counts equal to total board size then game is done 
...
def update_board(layout, move):
    ...
    for i in range(10): 
        for j in range(10): 
            if position_letter_grid_10[i][j] == move: 
                l[i][j] = "W"
                break
    return l
